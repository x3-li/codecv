## 選定基準
DGX Spark上で利用可能なアンチウイルス製品を選定する。
現時点では、各アンチウイルス製品の公式サイトにおいて、DGX OSへの対応が明確に記載されている製品は確認できていない。

そのため、以下の条件を満たす製品を候補とする。
- Ubuntu 24.04 LTSをサポートしていること
- Linux ARM64／AArch64アーキテクチャをサポートしていること
- DGX Sparkで使用されているKernel。 (Linux 6.1.x-xxxx-nvidia)に対応可能であること

なお、上記条件を満たす製品であっても、DGX Spark上での正式な動作を保証するものではない。
事前に各製品の営業員へ問い合わせを行い、以下の事項を確認する。
- NVIDIA DGX SparkおよびNVIDIA GB10への正式対応可否
- ARMv9-Aアーキテクチャへの対応可否
- NVIDIAカスタムKernelへの対応可否
- DGX Sparkとの互換性
- ライセンス費用
上記の確認結果および必要に応じた実機検証を踏まえ、最終的な導入製品を決定する。
