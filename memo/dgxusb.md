...
理由：
| 方式                        | 追加インストール | USBメモリ / 外付けHDD対策 | 偽装HID / USB LAN対策 | 一時的な有効化・無効化 | 本番運用への適性 |
| ------------------------- | -------: | ----------------: | ----------------: | ----------: | -------: |
| `usb-storage` の blacklist |       不要 |                可能 |                不可 |          低い |      限定的 |
| `udev` ルール                |       不要 |              一部可能 |                弱い |          普通 |       普通 |
| `sysfs` USB authorization |       不要 |                可能 |                可能 |          普通 |     やや複雑 |
| USBGuard                  |       必要 |                可能 |                可能 |          高い |       **推奨** |

補足：

- `usb-storage` の blacklist: USBストレージ系デバイスのみを無効化する簡易対策。USBキーボード偽装やUSB LANなどには対応できない。
- `udev` ルール: OS標準機能で柔軟に制御できるが、ルール設計が複雑になりやすく、抜け漏れが発生しやすい。
- `sysfs` USB authorization | Linuxカーネル標準のUSB認可機能を直接利用する方式。追加インストール不要だが、運用スクリプトや管理設計が必要。
- USBGuard: USBデバイスの認可制御をポリシーとして管理できる。デフォルト拒否、一時許可、監査、白リスト管理に向いている。
